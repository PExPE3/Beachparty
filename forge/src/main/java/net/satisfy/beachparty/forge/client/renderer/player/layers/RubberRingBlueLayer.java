...
public class RubberRingBlueLayer<T extends LivingEntity, M extends HumanoidModel<T>> extends RenderLayer<T, M> {

    private final RubberRingColoredModel<T> model;

    public RubberRingBlueLayer(RenderLayerParent<T, M> renderLayerParent) {
    ...
    public void render(@NotNull PoseStack poseStack, @NotNull MultiBufferSource multiBufferSource, int light, @NotNull T entity,
        ...
        if (entity instanceof Player) {
            ...
            if (ringStack.isEmpty() || !ringStack.is(net.satisfy.beachparty.core.registry.ObjectRegistry.RUBBER_RING_BLUE.get()))
                return;
            this.model.setupAnim(entity, limbSwing, limbSwingAmount, ageInTicks, netHeadYaw, headPitch);
            poseStack.pushPose();
            renderColoredCutoutModel(this.model, getTextureLocation(entity), poseStack, multiBufferSource, light, entity, 0.0f, 0.0f, 1.0f);
            poseStack.popPose();
        }
    }
    ...
    private static <T extends LivingEntity, M extends HumanoidModel<T>> void renderColoredCutoutModel(RubberRingColoredModel<T> model,
                                                                                                      ResourceLocation texture, PoseStack poseStack,
                                                                                                      MultiBufferSource buffer, int light, T entity,
